...
In this excerpt will discuss validation, authenticstion and authorization. We will be building on top of the previous excerpt. Refer to the previous material for the code snippet. You can also reference this material from [here](https://dev.to/otumianempire/basic-crud-api-with-express-2eoc).

> We are using the the snippet from the previous excerpt. You can follow that to have the starter code.
...
Before we jump into definitions of what, what is and all that, let me draw your attension to why we have to validate data received from users by referenceing some articles written on [DEV](dev.to). Check out these articles to have a different voice on why validations is importance. I think it will spare us the definition of validation.
...
-   For the _amount_, usually the question should be, do we consider a negative value or even zero in this case as an acceptable value for amount? What is the limit of the value that the user can pass? This is in a sense that, can a user pass 1 million or 1 billion?
-   For the _date_ value, we should consider the format of the expected expense date. We can have it that the date should start with the year followed by the month and then the day. The year, the month and the day can be separated by some specified character. We used hypen, `-`. So here you can specify whether the date should be separated by forward slash, `/`, or even a colon, `:`. You decide.
...
> It is also a good practice to separate concernces. If this sounds weird or strange then we can simply say that it is better to reduce the number of code in the `index.js` file.
...
For the sake of demonstration, we will create a user record for the authentication and since we have not done database integration yet, we will use an array of objects instead (we did the same for expenditures). Now, let's do some clean up. We are going to move the `expenditures` records (array) into a file and export it. Since it is acting as our data source, maybe we can name the file, `data.js`. After that, we'd add another variable for for `users`.
...
> "...", the three dots means that there are some code there. Perhaps the original

We will implement an endpoint to create a user and another, to log in users. There are some few things we have to consider:
...
-   The password will be the stored as the raw password (NOT BEST PRACTICE, JUST FOR DEMO)

### Sign up

To create a user, we first have to check the user data (array) and make sure that the email that the user is providing does not exist - there is no element in the array whose email matchs the provided email. When there is no user with such email, we then hash the password (for now just the raw password) and generate a UUID to uniquely identify the user.

These steps should help with the sign up process:

-   In `expense-tracker-api.http`, add a `post` request for user sign up
...
-   create a `post` request to handle user signup request. The route should match the one above, expecting _email_ and _password_ from the request body
-   **validate** the email and password
-   check the `users` array (from the data file) that there is no record with an email that matchs the email provided by the user. If such record exists, return an appropriate respond
...
-   as part of the succcess response, add the email and UUID and a message that says sign up successful or whatever appropriate message
...
This is the response that shiuld be expected assuming we are on the same page
...
Did you get any other response where the success was false? What was it and how did you fi it?

### Log in

To implement _login_, will check that there is a user with the provided email and that the User password matches password (hash) of the retrived record. Basically, the login will be much similar to the signup.

> Try implementing the login.

When a user logs in onto our platform, we have to find a way to allow the user to browse through the resources available to them without them having to log in all the time. What we are going to look at is how to **authorize** a user to CRUD their data (i.e expenses and user data). Usually we provide the user a means for them to access their data without having to login all the time. There are severals to do this. And some of them are:

-   **Basic authorization**: we send the user's credentials in every request they make, i.e their email in this case with the password. We would encode (base64) this credentials for security sake.
...
There are several complicated authroization schema out there. However, at this point, we will discuss each of these above forms of authorization.

-   **Basic auth**: The down side with this approach is that the user has to pass these encoded credentials all time in every request. It is practically logging in all the time. And the user's account can be compromised easily as well. The only way to resolve this is to ... (no I am not talking about this... too much work - don't do this, I hope we understand). Use any of the ones mentioned below.
-   **API key**: Just like the basic auth, api keys can be compromised. The advantage of an api key is that we can generate another api when it is compromised. We can have it that the user has to request new api keys every week or within some time frame. We set up a mechanisms to expire api keys so that the user will have to generate a new api key. Another down side of this is that, we have to save the api key somewhere so that we can match it against any request. (Use a cache)
-   **Bearer tokem (jwt)**: jwt is like an api token but the token itself can be time bounded. The token can expired and as such will become invalid after that period of expiry onwards. The token can store a payload which when parsed would be useful for identifying the owner of the token.

In most cases after login, an authorisation token is added as part of the response to the user. This token is then used by the user in every request thereafter to access the api resources. So Just to make this simple and to demonstrate how we can pass an api key for authrization, let's base64 encode the email and uuid passed as the auth token.

It is simple to encode a string to base64. Do, `Buffer.from(STRING TO ENCODE).toString("base64")`. So for our case we want to do, `email:uuid` as the string to encode.

So before the `return` statement, let's add the encoding and update the response to inlude the auth token.

```js
// sign up
app.post("/users/signup", (req, res) => {
    // some codes here were reducted
...
Run the sign up request again with another email and compare the difference in the response. Anything different? What was it?

Now we can do a proper login. The login request will not be that different from the sign up. It just the route that will be different.
...
Now we have an auth token. We will use the auth token to access resources only available to the owner of that token. To do that we have to modify and add (or update) the existing records to account for the `userId`. To make it simpler, I'd suggest you create multple users and updaten several or add more expenses with `userId`s.

I have two users and three expenses (statically) and as such I will updates them as is:
...
A protected endpoint or route is that which required some sort of "authorization" in order to access resources on that route. To protect an endpoint, in this case, we expect that the request to have an auth token and that we can trace the auth token to an authentic user.

The auth token can be passed in any of the means, discussed so far, that data can be passed in the request: body, param, query and headers. We'd would pass the auth token in the headers.

Let's modify the _List Expenditures_ endpoint to account for the auth token passed in the headers.

The request would look like this, below, when the auth token is added. Or rather, this is how we'd be passing the auth token in the headers.
...
We can get and log the content of the headers by doing
...
> `authorization` is the standard used when we have to pass auth tokens. Even though you just give it any key and it should still work fine.
...
In fact, this is what we did to generate the auth token
...
We have looked at how to do validations, authentication and authorization. What we have discussed here is roughly what what these concepts are and some "rough" way to implement them. For learning purposes, I believe these should do.

Some few things to consider:

-   Never save a raw password
-   Never use the auth mechnism employed here is production environment i.e encodeing the email and ID
...
In the next excerpt, we are going to look at how to use a library to validate data and also hash password and break our application down.
